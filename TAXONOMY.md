...
| `workflow-execution` | Workflow execution lifecycle emitted by `workflow-service` (e.g. execution completion) | Subject `evt.<tenant>.workflow-service.workflow.internal.native.execution_completed.v1` built by `buildExecutionCompletedSubject` and the envelope `producer: "workflow-service"` / `domain: "workflow"` (`services/workflow-service/src/temporal/activities/execution-completed-publisher.activity.ts:47,151-152`). tech is `platform` per §2 (Temporal is a runtime detail absent from the subject/envelope) |
...
| 19 | subject matches `evt.*.workflow-service.workflow.*` (producer token `workflow-service` AND domain token `workflow`) | `platform` | `workflow-execution` | `buildExecutionCompletedSubject` → `evt.<tenant>.workflow-service.workflow.internal.native.execution_completed.v1`; envelope `producer: "workflow-service"` / `domain: "workflow"` (`services/workflow-service/src/temporal/activities/execution-completed-publisher.activity.ts:47,151-152`). Evaluated BEFORE the catch-alls 16/17/18 (see design note) |
