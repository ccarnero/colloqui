...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "[run] Install Node >=18 (see integrations/mcp/mcp-repo-support-bot/README.md) and re-run." >&2
  exit 1
fi
if ! command -v npx >/dev/null 2>&1; then
  echo "[run] 'npx' was not found on PATH (usually ships with npm)." >&2
  echo "[run] Install Node >=18 / npm (see integrations/mcp/mcp-repo-support-bot/README.md) and re-run." >&2
  exit 1
fi
