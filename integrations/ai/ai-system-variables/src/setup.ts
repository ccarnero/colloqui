...
const AGENT_DESCRIPTION =
  ...
  "Brand-voiced priority classifier created by integrations/ai/ai-system-variables";
...
async function stageEnsureSystemVariables(): Promise<void> {
  ...
  await ensureSystemVariable(
    ...
    "Company name",
    "Brand name stamped into workflow notifications (integrations/ai/ai-system-variables)"
  );
  escalationVarId = await ensureSystemVariable(
    "escalationPriority",
    SYSVARS_ESCALATION_PRIORITY,
    "Escalation priority",
    "Priority level that routes to the escalation arm — PATCH this to re-route the workflow live (integrations/ai/ai-system-variables)"
  );
  await ensureSystemVariable(
    "brandVoice",
    SYSVARS_BRAND_VOICE,
    "Brand voice",
    "Writing style injected into the classifier agent's system prompt (integrations/ai/ai-system-variables)"
  );
}
