...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "[setup] Install Node >=18 (see integrations/ai/ai-agent-triage/README.md) and re-run." >&2
  exit 1
fi
if ! command -v npx >/dev/null 2>&1; then
  echo "[setup] 'npx' was not found on PATH (usually ships with npm)." >&2
  echo "[setup] Install Node >=18 / npm (see integrations/ai/ai-agent-triage/README.md) and re-run." >&2
  exit 1
fi
