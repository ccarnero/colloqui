...
# Drives the ai-agent-playground sample end-to-end — SDK-powered. Provisioning
# is now declarative (`manifest.yaml` + `yoizen manifests apply`, see
# README.md); the agent-resolution/execute/poll logic lives in src/index.ts,
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "[run] Install Node >=18 (see integrations/ai/ai-agent-playground/README.md) and re-run." >&2
  exit 1
fi
if ! command -v npx >/dev/null 2>&1; then
  echo "[run] 'npx' was not found on PATH (usually ships with npm)." >&2
  echo "[run] Install Node >=18 / npm (see integrations/ai/ai-agent-playground/README.md) and re-run." >&2
  exit 1
fi
