...
# the uploaded document — SDK-powered (sdk/GROWTH-PLAN.md P3.1). The actual
# provisioning logic (connector, KB, document upload + embedding wait, agent
# upsert/publish, runtime execution) lives in src/setup.ts, run via
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "[setup] Install Node >=18 (see integrations/ai/ai-knowledge-base-agent/README.md) and re-run." >&2
  exit 1
fi
if ! command -v npx >/dev/null 2>&1; then
  echo "[setup] 'npx' was not found on PATH (usually ships with npm)." >&2
  echo "[setup] Install Node >=18 / npm (see integrations/ai/ai-knowledge-base-agent/README.md) and re-run." >&2
  exit 1
fi
