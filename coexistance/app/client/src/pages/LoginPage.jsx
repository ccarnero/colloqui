...
const LoginPage = () => {
  ...
  const saved = JSON.parse(localStorage.getItem('coexistance_remember') || 'null')
  ...
  const handleSubmit = async (e) => {
    ...
    if (remember) {
      localStorage.setItem('coexistance_remember', JSON.stringify({ email, password }))
    } else {
      localStorage.removeItem('coexistance_remember')
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        <h1 className="text-2xl font-bold text-center mb-2">Coexistance</h1>
