...
* SSE: Server-Sent Events, NATS subscriber → browser streaming (replaced WebSocket)
* Event Bus: NATS — 9 primitives for messaging infrastructure
...
│   ├── PRD.md
│   └── help/                     # 8 guías (01-primeros-pasos a 07-troubleshooting)
├── server/
│   ├── package.json
│   ├── src/
│   │   ├── server.js             # Entry: Express + HTTP server + SSE + static
...
│   │   ├── bus/
│   │   │   └── # 9 primitives for NATS messaging
│   │   ├── services/
│   │   │   ├── sse/               # SSE bridge: subscribes to evt.*.coexistance.messaging.>, streams to browser
│   │   │   │   ├── index.js       # Entry point
│   │   │   │   ├── sse-bridge.js  # NATS subscriber → SSE formatter
│   │   │   │   └── routes.js      # GET /api/events/stream with JWT via query param
│   │   │   ├── ingress/           # Webhook processing
│   │   │   ├── egress/            # Message sending
...
* Recibir mensajes via webhook → MongoDB → NATS → SSE stream → UI en tiempo real
...
* **★ SSE filtering: solo procesa eventos de la cuenta activa (ignora otras)**
...
7. SSE ahora pasa por Vite proxy en dev (GET /api/events/stream con JWT via query param)
8. Tokens temporales de Meta duran 24h → solución: token-refresh endpoint + Account Settings UI
9. JWT claim es `sub`, no `userId` — consistente en create-token.js y sse-bridge.js
...
* `GET /api/events/stream?token=JWT` (dev via Vite proxy, prod same domain)
* Auth: JWT passed via query parameter
* Recibe: Server-Sent Events con `data: { type: 'new_message' | 'status_update', data: {...} }`
...
* Limpiar debug logs de SSE broadcast (dejar solo errores)
