...
Usá tu JWT token como parámetro query:
```
GET http://localhost:6666/api/events/stream?token=eyJhbG...
```

La conexión se establece inmediatamente y comienza a recibir eventos.
...
```
event: message_type
data: {"type":"...", "eventId":"...", "accountId":"...", "data": {...}}

```

Ejemplo — nuevo mensaje entrante:
```
event: new_message
data: {"type":"new_message", "eventId":"evt_123", "accountId":"acc_456", "data":{"contact_id":"...", "message": {...}}}
...
```
event: status_update
data: {"type":"status_update", "eventId":"evt_789", "accountId":"acc_456", "data":{"wa_message_id":"...", "status":"delivered"}}
...
La API EventSource maneja la reconexión automáticamente si la conexión se cae. El cliente (React hook `useEventStream`) se reconecta transparentemente.

### Cliente JavaScript / React

Usa el hook `useEventStream`:
```javascript
const { events, connected } = useEventStream('/api/events/stream');

// connected: boolean — estado de la conexión
// events: recibís eventos en tiempo real
```
...
**DEPRECADO**: El endpoint WebSocket anterior (`ws://localhost:6666/ws`) fue reemplazado por SSE. Use `/api/events/stream` en su lugar.
