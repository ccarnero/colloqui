...
import { CheckCircle, AlertTriangle, XOctagon, RefreshCw, ArrowLeft, Key, Info, Shield } from 'lucide-react'
...
const AccountSettingsPage = ({ account, onBack }) => {
  ...
  const handleSaveCredentials = async (e) => {
    e.preventDefault()
    if (!editAppId.trim() || !editAppSecret.trim()) return
    setSavingCreds(true)
    setMessage(null)
    const result = await api.updateMetaCredentials(account._id, editAppId.trim(), editAppSecret.trim())
    if (result.ok) {
      setMessage({ type: 'ok', text: `Credenciales actualizadas (App: ${result.data.meta_app_id})` })
      setEditAppId('')
      setEditAppSecret('')
    } else {
      setMessage({ type: 'error', text: `Error al guardar credenciales: ${result.error}` })
    }
          ...
          <>
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-5 mb-6">
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <dt className="text-gray-400">Meta App ID</dt>
                <dd className="text-gray-700 font-mono text-xs">{tokenStatus?.meta_app_id || '— (usa default del servidor)'}</dd>
              </dl>
            </div>

            {/* Meta App Credentials — for multi-app setups */}
            <div className="rounded-lg border border-gray-200 bg-white p-5 mb-6">
              <div className="flex items-center gap-2 mb-3">
                <Shield size={16} className="text-gray-400" />
                <h2 className="text-sm font-semibold text-gray-700">Credenciales de Meta App</h2>
              </div>
              <p className="text-xs text-gray-500 mb-4">
                Si esta cuenta usa una app de Meta distinta a la del servidor, actualizá las credenciales acá.
                Se usan para renovar tokens automáticamente.
              </p>
              <form onSubmit={handleSaveCredentials} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder={tokenStatus?.meta_app_id || 'ej. 1690770278576601'}
                    className="w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent font-mono"
                  />
                </div>
                <div>
                ...
                <button
                  type="submit"
                  disabled={savingCreds || !editAppId.trim() || !editAppSecret.trim()}
                  className="px-4 py-2 bg-gray-700 text-white rounded-md text-sm font-medium hover:bg-gray-800 disabled:opacity-50 transition-colors"
                >
