...
const SandboxForm = ({ onConnected, onBack }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 w-full max-w-md">
      <h2 className="text-lg font-semibold mb-4">Sandbox / Conexión Manual</h2>

      <form onSubmit={handleSubmit} className="space-y-4">
        {error && <div className="bg-red-50 text-red-700 text-sm rounded p-3">{error}</div>}

        <Field label="WABA ID *" value={wabaId} onChange={setWabaId} placeholder="ej. 123456789" required />
        <Field label="Phone Number ID *" value={phoneNumberId} onChange={setPhoneNumberId} placeholder="ej. 987654321" required />
        <Field label="Access Token *" value={accessToken} onChange={setAccessToken} placeholder="Tu token de Meta" required />
        <Field label="Teléfono" value={displayPhone} onChange={setDisplayPhone} placeholder="+1 555-0123 (opcional)" />
        <Field label="Nombre del negocio" value={businessName} onChange={setBusinessName} placeholder="Mi Negocio (opcional)" />

        {/* Collapsible Meta App credentials — for multi-app setups */}
        <div className="border-t border-gray-100 pt-3">
          ...
          >
            {showAdvanced ? '▾ Ocultar' : '▸ Credenciales de Meta App'} (opcional)
          </button>

          {showAdvanced && (
            <div className="mt-3 space-y-3 p-3 bg-gray-50 rounded-md">
              <p className="text-xs text-gray-400">
                Si esta cuenta pertenece a una app de Meta distinta a la configurada en el servidor,
                ingresá el App ID y App Secret acá. Se usarán para renovar tokens.
              </p>
              <Field label="Meta App ID" value={metaAppId} onChange={setMetaAppId} placeholder="ej. 1690770278576601" />
              <Field label="Meta App Secret" value={metaAppSecret} onChange={setMetaAppSecret} placeholder="ej. f0b2f677..." />
            </div>
          )}
        </div>
