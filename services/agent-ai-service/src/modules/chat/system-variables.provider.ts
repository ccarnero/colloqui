 ...
 * provider is a read-only consumer, reachable because `agent-ai-service`
 * uses the same `SharedTenantDatabaseMode.PerTenantDatabase` Postgres
 * connection scheme (same `postgres-credentials` secret, same per-tenant
 * database resolution) as `agent-admin-service` and `workflow-service`.
 * Mirrors `workflow-service/src/modules/workflows/system-variables.provider.ts`.
 */
@Injectable()
