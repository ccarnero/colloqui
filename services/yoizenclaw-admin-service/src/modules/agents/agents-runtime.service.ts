...
export class AgentsRuntimeService {
  private static readonly RUNTIME_RESPONSE_TIMEOUT_MS = 5 * 60 * 1000;
  ...
  async listMemoryProposals(
    ...
    const responseData = await this.requestRuntime(
      ...
      this.buildRuntimeActionEnvelope(
        ...
        correlationId,
        now,
        '//yoizenclaw-admin-service/admin/agents/memory-proposals/list',
      ),
  ...
  async reviewMemoryProposal(
    ...
    const responseData = await this.requestRuntime(
      ...
      this.buildRuntimeActionEnvelope(
        ...
        correlationId,
        now,
        `//yoizenclaw-admin-service/admin/agents/memory-proposals/${actionType}`,
      ),
  ...
  private buildRuntimeActionSubject(actionType: string): string {
    return `${YOIZENCLAW_SUBJECT_PREFIX}.${actionType}.v1`;
  }
  ...
  private buildRuntimeActionEnvelope(
    ...
    return {
      ...
      id: this.generateEventId(),
      source,
      type: `io.yoizen.yoizenclaw.${actionType}.request.v1`,
