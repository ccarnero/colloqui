...
function toRow(dto: AdapterDto): ExternalSourceRow {
  return {
    id: dto.id,
    adapter: {
      name: dto.name,
    ...
    status: dto.status,
    lastSync: dto.updatedAt
      ? new Date(dto.updatedAt).toISOString().slice(0, 16).replace("T", " ")
      : "—",
  };
}
...
@Component({
  ...
  template: `
    <div class="ws-header">
      <div>
        <div class="ws-title">External Sources</div>
        <div class="ws-subtitle">
          HTTP adapters for third-party services outside the cluster
        </div>
      </div>
...
export class ExternalSourcesComponent implements OnInit {
  ...
  readonly sources = signal<ExternalSourceRow[]>([]);
  ...
  private loadSources(): void {
    this.adapterService.list("external").subscribe({
      next: (dtos) => {
