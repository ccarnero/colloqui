...
@Component({
    ...
    <nav class="palette" [class.collapsed]="collapsed()" aria-label="Agent builder">
      ...
      <section class="palette-group">
        ...
        @if (!collapsed() && skillsExpanded()) {
          @for (skill of skills(); track $index) {
            ...
            >
              ...
              @if (skill.source === "catalog") {
                <span class="catalog-chip">catalog</span>
              }
              <button
                type="button"
                class="remove-btn"
          ...
          <button
            type="button"
            class="palette-add-btn ghost"
            (click)="addSkillFromCatalog.emit()"
          >
        ...
        @if (!collapsed() && toolsExpanded()) {
          @for (tool of tools(); track $index) {
            <button
              ...
              [class.builtin-item]="tool.sourceType === 'builtin'"
            ...
            >
              ...
              @if (tool.sourceType === "builtin") {
                <span class="builtin-chip">builtin</span>
              }
              @if (tool.sourceType !== "builtin") {
                <button
                  type="button"
        ...
        <button
          type="button"
          class="palette-item"
          [class.active]="isActive(builtinTools)"
          (click)="select.emit(builtinTools)"
          [matTooltip]="collapsed() ? 'Built-in Tools' : ''"
          matTooltipPosition="right"
        >
          <mat-icon class="row-icon">memory</mat-icon>
          <span class="row-label">Built-in Tools</span>
        </button>

        <button
          type="button"
          class="palette-item"
          [class.active]="isActive(mcpServers)"
          (click)="select.emit(mcpServers)"
          [matTooltip]="collapsed() ? 'MCP Servers' : ''"
          matTooltipPosition="right"
        >
          <mat-icon class="row-icon">hub</mat-icon>
        ...
        <button
          type="button"
          class="palette-item"
          [class.active]="isActive(variables)"
          (click)="select.emit(variables)"
          [matTooltip]="collapsed() ? 'Variables' : ''"
          matTooltipPosition="right"
        >
          <mat-icon class="row-icon">input</mat-icon>
        ...
        <button
          type="button"
          class="palette-item"
          [class.active]="isActive(knowledgeBases)"
          (click)="select.emit(knowledgeBases)"
          [matTooltip]="collapsed() ? 'Knowledge Bases' : ''"
          matTooltipPosition="right"
        >
          <mat-icon class="row-icon">library_books</mat-icon>
          <span class="row-label">Knowledge Bases</span>
        </button>
        ...
        @if (editingAgentId()) {
          <button
            ...
            (click)="select.emit(versions)"
            [matTooltip]="collapsed() ? 'Versions' : ''"
            matTooltipPosition="right"
          >
            <mat-icon class="row-icon">history</mat-icon>
            <span class="row-label">Versions</span>
          </button>
        }
      </section>
  ...
  styles: `
    ...
    .palette-add-btn.ghost:hover {
      background: var(--accent-dim, rgba(99, 102, 241, 0.08));
      color: var(--text-primary);
      border-color: var(--primary, #6366f1);
    }

    .palette-add-btn.ghost mat-icon {
      font-size: 16px;
      width: 16px;
      height: 16px;
    }
...
export class AiAgentEditorNavComponent {
  ...
  protected readonly builtinTools = SELECTION_BUILTIN_TOOLS;
  protected readonly mcpServers = SELECTION_MCP_SERVERS;
  protected readonly variables = SELECTION_VARIABLES;
  protected readonly knowledgeBases = SELECTION_KNOWLEDGE_BASES;
  protected readonly versions = SELECTION_VERSIONS;
