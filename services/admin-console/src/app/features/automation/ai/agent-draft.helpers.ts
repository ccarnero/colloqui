...
import { getAgentLlmConfig } from "../../../core/models/agent.model";
import { mapSubagentConfigToDraft, parseToolPayload } from "./ai.helpers";
...
const STORAGE_PREFIX = "ai:agent-draft:";
// Bump this when the IAgentDraft shape or comparison logic changes —
...
export function buildDraftFromAgent(
  ...
  const subagents: ISubagentDraft[] =
  ...
  const inputVariables: VariableDeclaration[] =
    (agent.input_variables ?? []).map((v) => ({ ...v }));
  const outputVariables: VariableDeclaration[] =
    (agent.output_variables ?? []).map((v) => ({ ...v }));
