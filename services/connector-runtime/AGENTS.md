# AGENTS.md - connector-runtime

## Project Overview

The connector-runtime is a standalone Temporal worker that executes generic HTTP-facing activities on the `connector-runtime` task queue. It processes `endpointCall` and `serviceCall`. When adapter-managed configuration is present, it resolves the full request configuration (URL, auth, headers, retries, timeouts) through `AdapterClient` backed by a Redis stale-while-revalidate cache. Otherwise, it makes direct HTTP requests. All HTTP calls use `tracedFetch` from `@yoizen/observability` for OpenTelemetry instrumentation. It has no NestJS framework — just a raw Temporal worker with an HTTP health server.
...
| File | Purpose |
|------|---------|
| `src/worker.ts` | Creates Temporal `NativeConnection`, registers activities, starts worker on `connector-runtime` task queue, runs health server |
...
```
Temporal Server
  -> deliver endpointCall / serviceCall activity task (connector-runtime queue)
