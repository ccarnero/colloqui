...
class TestBuildInternalAgentEnvelopeStructure:
    ...
    def test_data_contains_payload(self) -> None:
        ...
        assert envelope.data["payload_inline"] is True
        assert envelope.data["payload_ref"] is None
        assert envelope.data["payload_bytes"] > 0
        assert envelope.data["payload_checksum"]
