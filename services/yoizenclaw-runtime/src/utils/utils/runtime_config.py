...
class RuntimeConfigRepository:
    ...
    async def load_all_agent_configs(self) -> dict[str, dict[str, object]]:
        ...
        return {str(row["agent_id"]): self._coerce_dict(row["config"]) for row in rows}
    ...
    async def save_agent_config(
        ...
        async with self._memory.pool.acquire() as conn:
            await conn.execute(
                ...
                (id, tenant_id, agent_id, config, created_at, updated_at)
                VALUES ($1, $2, $3, $4, $5, $5)
        ...
        async with self._memory.pool.acquire() as conn:
            for file_payload in files:
                ...
                await conn.execute(
                    ...
                    (id, name, tenant_id, path, category, content, format, created_at, updated_at)
                ...
                await self._sync_runtime_table_for_file(
                    conn,
                    file_path,
                    content,
                    resolved_tenant_id,
        ...
        if self._is_channels_file(file_path):
            ...
            for item in channels:
                ...
                await conn.execute(
                    ...
                    (id, tenant_id, channel, agent_id, display_name, config, created_at, updated_at)
