...
const sqlProvider: FactoryProvider<Sql> = {
  ...
  useFactory: (): Sql => {
    const host =
      process.env.POSTGRES_HOST ?? 'postgres.support-services-dev.svc.cluster.local';
...
class SchemaInitializer implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.log('Registry schema ensured');
    } catch (err) {
      this.logger.error('Failed to ensure registry schema', err);
    }
  }
