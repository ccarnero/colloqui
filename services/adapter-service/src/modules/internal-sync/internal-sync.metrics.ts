 ...
 * (`adapter-service-api` vs `adapter-service-worker`) so Prometheus
 * splits the series by deployment role automatically.
 */
const meter = getMeter(resolveServiceName("adapter-service"));
...
export const adapterInternalSyncRunnerMetrics: INatsConsumerMetricsSink =
  createNatsConsumerMetrics(resolveServiceName("adapter-service"));
