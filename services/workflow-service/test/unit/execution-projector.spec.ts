...
function makeMsg(
  ...
  const subject = `evt.${tenant}.workflow-service.workflow.internal.native.execution_completed.v1`;
  const envelope = {
    specversion: "1.0",
    id: `evt-${executionId}`,
...
describe("ExecutionProjectorService", () => {
  ...
  beforeEach(() => {
    applyStatusBatch = mock(
      (_tenantId: string, rows: { id: string; status: string }[]) =>
  ...
  it("flushes a batch after MAX_BATCH_WAIT_MS", async () => {
    const p1 = (svc as unknown as IProjectorInternals).enqueue(
      makeMsg("e1", "COMPLETED"),
    );
    const p2 = (svc as unknown as IProjectorInternals).enqueue(
      makeMsg("e2", "FAILED"),
    );
    ...
    const [tenantId, batch] = applyStatusBatch.mock.calls[0];
  ...
  it("flushes immediately once the batch reaches MAX_BATCH_SIZE (100)", async () => {
    ...
    for (let i = 0; i < 100; i++) {
      promises.push(
        (svc as unknown as IProjectorInternals).enqueue(
          makeMsg(`e${i}`, "COMPLETED"),
        ),
      );
    }
  ...
  it("splits 150 messages for the same tenant into two batches (100 + 50)", async () => {
    const promises: Promise<void>[] = [];
    for (let i = 0; i < 150; i++) {
      promises.push(
        (svc as unknown as IProjectorInternals).enqueue(
    ...
    const first = applyStatusBatch.mock.calls[0][1];
    const second = applyStatusBatch.mock.calls[1][1];
    expect(first.length).toBe(100);
    expect(second.length).toBe(50);
  });

  it("partitions messages from different tenants into separate batches", async () => {
    const pA1 = (svc as unknown as IProjectorInternals).enqueue(
      makeMsg("a1", "COMPLETED", "tenant-a"),
    );
    const pA2 = (svc as unknown as IProjectorInternals).enqueue(
      makeMsg("a2", "FAILED", "tenant-a"),
    );
    const pB1 = (svc as unknown as IProjectorInternals).enqueue(
      makeMsg("b1", "COMPLETED", "tenant-b"),
    );

    await Promise.all([pA1, pA2, pB1]);

    expect(applyStatusBatch).toHaveBeenCalledTimes(2);
    const byTenant = new Map<string, { id: string; status: string }[]>();
    for (const call of applyStatusBatch.mock.calls) {
      byTenant.set(call[0] as string, call[1]);
    }
    expect(byTenant.get("tenant-a")?.length).toBe(2);
    expect(byTenant.get("tenant-b")?.length).toBe(1);
  });

  it("rejects every pending promise in a tenant batch when the repo throws — other tenants unaffected", async () => {
    applyStatusBatch.mockImplementation(
      (tenantId: string, rows: { id: string; status: string }[]) => {
    ...
    const results = await Promise.allSettled([pA1, pA2, pB1]);
  ...
  it("resolves without touching the repo when message is unparseable", async () => {
    ...
    await (svc as unknown as IProjectorInternals).enqueue(bad);
  ...
  it("resolves without touching the repo when payload lacks executionId/status", async () => {
    ...
    await (svc as unknown as IProjectorInternals).enqueue(msg);

    expect(applyStatusBatch).toHaveBeenCalledTimes(0);
  });

  it("resolves without touching the repo when subject is malformed (no tenant token)", async () => {
    const msg = {
      subject: "evt.malformed",
      data: new TextEncoder().encode(
        JSON.stringify({ data: { payload: { executionId: "x", status: "X" } } }),
      ),
    } as unknown as JsMsg;
