...
describe("ClientsService", () => {
  ...
  describe("list", () => {
    it("should return all active clients when no tenantId", async () => {
      const rows = [
        { id: "c1", client_id: "yoizen_abc", name: "a", scope: "platform", created_at: new Date(), updated_at: new Date() },
      ];
    ...
    it("should filter by tenant scope when tenantId provided", async () => {
      const rows = [
        { id: "c1", client_id: "yoizen_abc", name: "a", scope: "tenant:demo", created_at: new Date(), updated_at: new Date() },
      ];
      (sql as unknown as ReturnType<typeof mock>).mockResolvedValueOnce(rows);
  ...
  describe("revoke", () => {
    ...
    it("should revoke an active client", async () => {
      (sql as unknown as ReturnType<typeof mock>).mockResolvedValueOnce([{ id: "c1" }]);
