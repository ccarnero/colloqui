...
describe("TokenService", () => {
  ...
  describe("onModuleInit", () => {
    it("should throw if JWT_SECRET is missing", () => {
      ...
      const svc = new (TokenService as new (...args: unknown[]) => TokenService)(sql);
  ...
  describe("clientCredentials", () => {
    ...
    it("should throw UnauthorizedException for deactivated client", async () => {
      (sql as unknown as ReturnType<typeof mock>).mockResolvedValueOnce([
        { id: "c1", client_secret_hash: "hash", scope: "platform", is_active: false },
      ]);
      await expect(
    ...
    it("should throw UnauthorizedException for wrong secret", async () => {
      ...
      (sql as unknown as ReturnType<typeof mock>).mockResolvedValueOnce([
        { id: "c1", client_secret_hash: hash, scope: "platform", is_active: true },
      ]);
      await expect(
  ...
  describe("login", () => {
    ...
    it("should return tokens for valid platform user", async () => {
      ...
      (sql as unknown as ReturnType<typeof mock>).mockResolvedValueOnce([
        { id: "u1", email: "admin@test.com", password_hash: hash, role: "admin", is_active: true },
      ]);
    ...
    it("should throw for deactivated platform user", async () => {
      ...
      (sql as unknown as ReturnType<typeof mock>).mockResolvedValueOnce([
        { id: "u1", email: "admin@test.com", password_hash: hash, role: "admin", is_active: false },
      ]);
