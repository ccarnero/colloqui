...
## What is HTTP Adapter?

HTTP Adapter listens on the `http-adapter` Temporal task queue for activity tasks. Each activity represents a single HTTP request that may be configured in one of two ways:
...
- Registers activities on the `http-adapter` task queue
...
```
Temporal Server receives activity task for http-adapter queue
         ↓
...
The service is deployed as a plain Kubernetes Deployment (not Knative Service) because Temporal uses pull-based task distribution. KEDA monitors the `http-adapter` task queue depth and scales accordingly:
...
Adapters are managed by adapter-service. HTTP Adapter queries adapter-service for runtime configuration:
...
- **workflow-service**: `endpointCall` actions dispatch to http-adapter task queue
- **event-processor**: HTTP enrichment and webhook delivery use http-adapter
- **webhook-service**: Webhook delivery uses http-adapter
- External callers: Any Temporal-enabled client can dispatch to http-adapter queue
