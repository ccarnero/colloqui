# AGENTS.md - HTTP Adapter

## Project Overview

The HTTP Adapter is a standalone Temporal worker that executes generic HTTP-facing activities on the `http-adapter` task queue. It processes `endpointCall` and `serviceCall`. When adapter-managed configuration is present, it resolves the full request configuration (URL, auth, headers, retries, timeouts) through `AdapterClient` backed by a Redis stale-while-revalidate cache. Otherwise, it makes direct HTTP requests. All HTTP calls use `tracedFetch` from `@yoizen/observability` for OpenTelemetry instrumentation. It has no NestJS framework — just a raw Temporal worker with an HTTP health server.
...
| File | Purpose |
|------|---------|
| `src/worker.ts` | Creates Temporal `NativeConnection`, registers activities, starts worker on `http-adapter` task queue, runs health server |
...
| `src/activities/service-call.activity.ts` | Resolves internal service calls via adapter mirror first, then falls back to `registry-service` DNS lookup |
...
```
Temporal Server
  -> deliver endpointCall / serviceCall activity task (http-adapter queue)
...
- Image: `dev.local/http-adapter:local`
...
|---------|-------------|
| **Temporal Server** | Receives activity tasks from the `http-adapter` task queue |
...
| **`@yoizen/shared`** | `HTTP_ADAPTER_TASK_QUEUE`, `HttpEndpointRequest`, `HttpServiceRequest`, `TENANT_HEADER`, `AdapterClient`, `DEFAULT_ADAPTER_SERVICE_URL` |
