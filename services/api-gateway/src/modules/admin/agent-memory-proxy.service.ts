...
export interface IAgentMemoryProxyOptions {
  method: string;
  path: string;
  tenantId: string;
...
@Injectable()
export class AgentMemoryProxyService {
  private readonly logger = new PinoLoggerService(AgentMemoryProxyService.name);
  private readonly baseUrl: string;

  constructor() {
    this.baseUrl = gatewayConfig.services.agentMemory;
  }

  async proxy(options: IAgentMemoryProxyOptions): Promise<object> {
    ...
    if (!res.ok) {
      await throwProxyError(res, "Agent memory service", this.logger);
    }
