...
export class JobsService {
  ...
  async run(tenantId: string, id: string): Promise<IJobExecution> {
    ...
    // Publish NATS event so agent-ai-service picks up the execution
    try {
      await this.natsPublisher.publishJobTrigger({
        tenantId,
        jobId: job.id,
        executionId: execution.id,
        eventPayload: { agent_id: job.agent_id, ...(job.payload as Record<string, unknown>) },
      });
    } catch (error) {
      this.logger.error(
        `Failed to publish job.trigger event for manual run of job '${job.id}'`,
        error,
      );
      // Swallow errors so the operation still succeeds
    }
