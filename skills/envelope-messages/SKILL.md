...
- Real bus producers: **eleven** — seven named by a shared constant (`CHANNEL_PRODUCER`, `REGISTRY_PRODUCER`, `AGENT_ADMIN_PRODUCER`, `AGENT_AI_PRODUCER`, `AGENT_MEMORY_PRODUCER`, `AGENT_SCHEDULER_PRODUCER`, `AI_AGENT_GATEWAY_PRODUCER` — all in `packages/shared/src/constants.ts` except `CHANNEL_PRODUCER`, which is in `channel.constants.ts`) and five spelled as literals in the publishing service: `"api-gateway"`, `"agent-ai-service"`, `"connector-runtime"`, `"workflow-service"` and `"provisioning-service"` (the last via a file-local `const PRODUCER`). `AGENT_AI_PRODUCER` arrived with the E3 subject migration (2026-08-07, `PENDIENTES/04-e3-subject.spec.md`) so agent-ai-service's subject tokens and envelope `producer` come from one constant; the bare `"agent-ai-service"` literal survives on the runtime-token publish path, which is why the service appears in BOTH lists. Channel constant: `CHANNEL_PRODUCER = "channel-service"`. Matches `DOCS/messaging/envelope.md` §2.1. Reproduce the census with:
  ```bash
  rg -o --no-filename 'producer: [A-Z_]+PRODUCER' -g '*.ts' -g '!*.spec.ts' -g '!**/test/**' services packages | sort -u   # 7
  rg -o --no-filename 'producer: "[a-z-]+"'      -g '*.ts' -g '!*.spec.ts' -g '!**/test/**' services packages | sort -u   # 5, two of which
                                                                                                                          # duplicate AGENT_ADMIN_PRODUCER
                                                                                                                          # and AGENT_AI_PRODUCER
