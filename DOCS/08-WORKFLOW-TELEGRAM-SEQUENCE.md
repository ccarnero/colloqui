...
- Causal chain (`causation_id`, `correlation_id`, `transport.depth`) is extracted from the incoming `ChannelEnvelope` and propagated through the workflow context so downstream `channelSend` envelopes comply with DOCS/arquitectura/02 §6.
