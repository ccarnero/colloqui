# Documentation/code validation test plan (proposal)

Class: future
Summary: The still-unimplemented half of the doc/code validation plan — proposed test categories, fixtures, implementation order and CI triggers that no code implements yet.

> **FUTURE — nothing here is implemented.** Split out of
> `DOCS/guides/doc-code-validation-tests.md` by the docs-truth-audit T10
> (ruling D9): that file carried a shipped guard script and an unbuilt test plan
> under one heading. What IS shipped lives in
> [`../guides/doc-code-guards.md`](../guides/doc-code-guards.md); this file is
> the remaining proposal. Promotion rule: when a section here lands, move it
> into the guides doc in the same commit that lands it.
