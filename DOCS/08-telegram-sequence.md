...
1. **api-gateway** receives the raw HTTP POST, wraps it in a `WebhookIngressEnvelope`, and publishes it to `INGRESS-<tenant>` (`evt.<tenant>.api-gateway.messaging.telegram.webhook.webhook_received.v1`). No signature verification happens here.
...
- Causal chain (`causation_id`, `correlation_id`, `transport.depth`) is extracted from the incoming `ChannelEnvelope` and propagated through the workflow context so downstream `channelSend` envelopes comply with DOCS/es/arquitectura/02 §6.
- Large webhook payloads (>256 KB) are transparently inflated by the claim-check middleware in `MultiTenantConsumerManager.wrapHandler` before any handler sees the message.
