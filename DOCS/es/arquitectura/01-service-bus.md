...
| `PLATFORM_TENANTS` | JetStream stream | `platform.tenant.>` | Ciclo de vida de tenants — `platform.tenant.provision.requested` lo consume el durable `tenant-provisioner` de tenant-service | `packages/shared/src/tenant-events.ts` |
