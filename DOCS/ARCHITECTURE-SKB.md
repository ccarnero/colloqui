...
```typescript
{
  skbContainerId: string;
  skbFileId: string;       // DB UUID
  tenantId: string;
  fileId: string;           // caller-assigned logical file ID
  fileBase64: string;       // raw file content (base64)
  filename: string;         // original filename for extension detection
  categories: string[];     // optional tag filter
  sheetName?: string;       // optional Excel sheet
}
```
...
See [SKB-API.md](./SKB-API.md) for the full API reference with request/response examples, error codes, and rate limiting details.
