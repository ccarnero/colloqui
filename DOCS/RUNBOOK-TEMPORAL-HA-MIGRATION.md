...
The bootstrap scripts (`bootstrap-minikube.sh`, `bootstrap-orbstack.sh`)
...
[`bootstrap-minikube.sh`](../bootstrap-minikube.sh) and
[`bootstrap-orbstack.sh`](../bootstrap-orbstack.sh) do a
...
[`infrastructure/base/temporal/cronjob-shard-rebalancer.yaml`](../infrastructure/base/temporal/cronjob-shard-rebalancer.yaml)
runs every 2 min:
...
A typo in
[`infrastructure/base/temporal/configmap-docker.yaml`](../infrastructure/base/temporal/configmap-docker.yaml)
...
[`deployment-worker.yaml`](../infrastructure/base/temporal/deployment-worker.yaml)
...
git checkout <pre-migration-sha> -- \
  ...
  bootstrap-minikube.sh bootstrap-orbstack.sh scripts/purge-temporal.sh
