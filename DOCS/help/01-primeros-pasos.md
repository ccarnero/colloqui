...
Esto llama a `POST /tenants` para crear el tenant (namespace K8s + Postgres dedicado), y luego registra un usuario admin en `auth-service`.
...
Los IDs los obtenés del Meta Developer Dashboard — ver [02-meta-dashboard.md](./02-meta-dashboard.md).
...
- Recibir mensajes: ver [DOCS/flujos/01-recibir-mensaje.md](../flujos/01-recibir-mensaje.md)
