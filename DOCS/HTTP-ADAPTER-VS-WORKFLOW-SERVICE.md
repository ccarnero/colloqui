# HTTP Adapter vs Workflow Service: Integration Patterns

A practical guide for developers choosing between HTTP Adapter and Workflow Service for their use cases, with decision matrices, scenario walkthroughs, and example implementations.

## Quick Decision Matrix

| Need | Use HTTP Adapter | Use Workflow Service | Use Agent Call |
|------|------------------|----------------------|-----------------|
...
| High-throughput HTTP | ✅ Yes | ✓ Via http-adapter | ✗ No |
| Adapter-driven config | ✅ Yes | ✓ Via http-adapter | ✗ No |
...
### When to Use HTTP Adapter Directly
...
**Usage Pattern**:
- **Called by**: event-processor, webhook-service, any Temporal client
- **No workflow**: Direct activity dispatch to http-adapter task queue
- **Pseudocode**:
...
// OR dispatch to http-adapter queue directly
...
- ✅ Adapter config caching in Redis
...
### HTTP Adapter: Immediate Retries
...
```
// http-adapter handles retries automatically
...
### When HTTP Adapter Outperforms Workflow
...
Use case: Real-time CRM enrichment
  ...
  Option A: Workflow Service
    ...
    ├─ Workflow dispatch to http-adapter (10ms)
    ├─ http-adapter picks up (wait ~30ms)
    ├─ Execute HTTP call (20ms)
    ├─ Return result (10ms)
    └─ Total: ~120ms (FAILS latency SLA)
  
  Option B: Direct HTTP Adapter
    ├─ Dispatch to http-adapter queue (5ms)
    ├─ http-adapter picks up (wait ~10ms)
    ├─ Execute HTTP call (20ms)
    ├─ Return result (5ms)
...
|-----------|-------------|----------------|---------|
| **HTTP Adapter** | 200 activities/replica | 200 req/sec (1 replica) | 20-50ms |
...
**Recommendation**:
- Use **HTTP Adapter directly** for: Real-time, high-throughput, low-latency needs
- Use **Workflow Service** for: Complex orchestration, state management, monitoring, audit trail

---

## Scenario 6: Adapter-Driven Configuration

### Both Services Support Adapter Config

```
HTTP Adapter:
  Caller provides: adapterId, endpointId, optional params
  Service resolves: URL, auth, headers, timeout, retries
  Cache: Redis SWR (300s TTL, 60s stale window)
  
Workflow Service (via http-adapter):
...
| Aspect | HTTP Adapter | Workflow Service |
|--------|--------------|------------------|
| **Adapter config resolution** | Direct | Via endpointCall action |
| **Caching strategy** | Redis SWR | Same (http-adapter caches) |
...
- [ ] Adapter-driven config simplifies integration
...
### Pattern 1: Workflow that Calls HTTP Adapter
...
**Benefit**: State management + adapter integration

### Pattern 2: HTTP Adapter with Retry Logic
