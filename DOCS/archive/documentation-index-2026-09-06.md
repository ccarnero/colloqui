# Documentation index snapshot — 2026-09-06

Class: RECORD
Summary: Preserved pre-consolidation DOCS index, including unverified operational and architecture details.
Status: historical

Frozen source: DOCS/README.md before the 2026-09-06 consolidation. The content
below is preserved for traceability, not validated current instructions. Relative
links were adjusted for this archive location. Follow the current
[documentation index](../README.md) and its canonical targets for new work.
Original document metadata below describes the old source, not this record.
...
Class: descriptive
Summary: Documentation index, developer quickstart, and navigation to the current engineering workflow and guard modes.
...
Start with [AGENTS.md](../../AGENTS.md), the [loop index](../../manual-loops/README.md),
[SPEC authoring](../../manual-loops-templates/README.md), and the
[working register](../../PENDIENTES/README.md). The [playbook](../guides/LOOP-PLAYBOOK.md)
links the current execution procedure. FP-scoped work also follows the
[FP delivery guide](../guides/manual-loop-fp-delivery.md); completed loops remain
historical evidence.
...
| Tier | Holds | Kind |
|---|---|---|
| [`AGENTS.md`](../../AGENTS.md) (repo root) | The constitution — rules, binding styles, review standards. The single normative repository contract | Prescriptive |
...
The full guard audit rejects a resurrected per-component AGENTS.md file.

### Enforcement (G0)

`scripts/checks/doc-code-guards.sh` defaults to KISS: G6a, G6b, G7, G9,
G15, G16, G17 and G18. `--full` runs every registered guard, including the
broader documentation and diff-based checks. Selected guards accumulate failures
and return a non-zero exit status if any failed.

```bash
./scripts/checks/doc-code-guards.sh          # KISS default
./scripts/checks/doc-code-guards.sh -v       # verbose KISS
./scripts/checks/doc-code-guards.sh --full   # explicitly scoped broader audit
```

The [guard guide](../guides/doc-code-guards.md) documents individual checks and
coverage limits. The [CI workflow](../../.github/workflows/engineering-guards.yml)
runs KISS on pull requests and pushes. Its presence does not prove successful
remote execution or required branch protection. G9 examines locally modified
files, so a clean CI checkout supplies no DI-change coverage through that guard.
Service tests, integration checks and independent review remain task-specific.
