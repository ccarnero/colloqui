...
Class: descriptive
Summary: How the Structured Knowledge Base works as built inside agent-admin-service — data model, API surface, ingestion pipeline, LLM schema analysis, query pipeline, configuration and troubleshooting.
...
Upload is live end to end: the api-gateway's `AdminStructuredKBController` proxies
`POST containers/:id/files` to `SKBContainersController.uploadFile`
(`@Post(":id/files")`, `@HttpCode(ACCEPTED)`), which validates the extension and
base64, creates the `skb_files` row and publishes the ingestion event consumed by
`SKBIngestionWorkerService`.

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/admin/structured-kb/containers/:id/files` | Implemented — 202 `{ fileId, status }` |

> **Note**: list-files, delete-file and get-file-schema have **no route at all** —
> not on `SKBContainersController`, not on `StructuredKBController`, and no
> api-gateway proxy. Calling them is a 404. File status is queryable via the
> `skb_files` table directly.
...
// Upload file — the class is UploadSKBFileDto (dto/upload-skb-file.dto.ts).
// There is NO caller-supplied `file_id`: the controller generates it with
// randomUUID(). The same file also exports SKB_SUPPORTED_EXTENSIONS.
...
export class QuerySKBDto {
  ...
  limit?: number;          // controller default when omitted: 10
...
```typescript
{
  ...
  fileId: string;        // file id, generated by the controller (required)
  tenantId: string;      // tenant slug (required)
  kbId?: string;         // job-tracking KB id; defaults to containerId
  fileUrl?: string;      // see the naming caveat below
  categories: string[];  // forwarded from the upload request
  sheetName: string | null;
}
```

Missing `containerId`/`fileId`/`tenantId` → `PermanentError` → term + per-tenant DLQ.

> **`fileUrl` is a misnomer.** `publishSkbFileIngestion` sets
> `fileUrl: payload.fileBase64` — the file content rides **inline as base64** in the
> event; nothing is fetched from a URL. The publisher is
...
> query path (`SKBRowsRepository.executeQuery()`, which is what actually runs;
> `SKBQueryService.buildSql()` builds the same string only for the returned `sql`
> field and the history row) interpolates `limit` directly with no `Math.min` cap.
> Enforcement is `QuerySKBDto`'s `@Min(1) @Max(1000)` plus a hand-written range
> check in `StructuredKBController`, and the effective default when `limit` is
> omitted is **`10`** (`body.limit ?? 10` in the controller) — `SKBQueryService`'s
> own `options?.limit ?? 100` fallback never fires on the HTTP path. `enforceLimit()`
> in `skb-sql-safety.ts` exists but is never called from the live query path.
...
Moved to [`design-decisions.md`](./design-decisions.md) (docs-truth-audit T10,
ruling D8): it is a decision record — the options weighed and the one chosen —
not a description of the running system. The shipped result is the `data JSONB`
+ GIN + generated `tsvector` schema in §2.1 above.
...
├── skb-file-parser.ts
├── skb-sql-safety.ts
├── skb-llm.config.ts                    ← added (shared SKB_LLM_PROVIDER/MODEL_ID + credential resolution)
...
└── dto/
    ...
    ├── query-skb.dto.ts
    ├── update-skb.dto.ts
    └── upload-skb-file.dto.ts           ← added (UploadSKBFileDto + SKB_SUPPORTED_EXTENSIONS)
```

(21 files + `types/` + `dto/`, matching `ls src/modules/structured-kb`.)
...
| `src/providers/nats.provider.ts` | Add `publishSkbFileIngestion()` (lower-case `kb` — the shipped spelling) |
...
Moved to [`design-decisions.md`](./design-decisions.md) (docs-truth-audit T10,
ruling D8): the risk table is design-time analysis, not as-built behaviour — its
§9.1 SQL-injection row describes a defence the query path does not actually
call. The as-built security position is [`security.md`](./security.md).
...
> **Reality check (2026-08-02)**: the 5 wiring defects tracked in §8.1
> (`skb_container_files`/`skb_files` mismatch, `insertRows` signature + column
> mismatches, watchdog stuck-file lookup, `skb_query_history` never written,
> hardcoded LLM provider) and §4.3 are fixed, and the last gap — a missing
> upload route — is closed too: `SKBContainersController.uploadFile` publishes
> `skb_file_ingestion.v1` (§4.2). Upload → parse → analyse → insert runs end to
> end. What is still missing is read-side surface (no list/delete/schema file
> routes) and observability (no `skb_*` metrics).
...
| `POST` | `/admin/structured-kb/containers/:id/files` | `SKBContainersController` (gateway proxies it) | Implemented (202) |
| `POST` | `/admin/structured-kb/containers/:id/query` | `StructuredKBController` (gateway proxies it) | Implemented, `SKBRateLimitGuard` |

Seven routes, and that is the whole SKB HTTP surface. `GET .../files`,
`DELETE .../files/:fileId` and `GET .../files/:fileId/schema` are **not
implemented anywhere** — neither service declares them.
