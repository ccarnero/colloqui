...
| [`bootstrap-minikube.sh`](../bootstrap-minikube.sh) / [`bootstrap-orbstack.sh`](../bootstrap-orbstack.sh) | Wait for the new CNPG cluster in parallel with the others, then invoke `ensure-temporal-visibility-schema.sh` **before** the Temporal rollout. Means a fresh `./bootstrap-*.sh dev support-services` Just Works — no manual step. |
...
Both bootstrap scripts now invoke [`infrastructure/scripts/ensure-temporal-visibility-schema.sh`](../infrastructure/scripts/ensure-temporal-visibility-schema.sh) automatically. It:
...
**Easiest path: re-run the bootstrap script.** `bootstrap-minikube.sh` and `bootstrap-orbstack.sh` already include the wait for `cluster/postgres-temporal-visibility` and the call to `ensure-temporal-visibility-schema.sh`, so this is enough:

```bash
./bootstrap-minikube.sh dev support-services    # or bootstrap-orbstack.sh
```
