...
- [x] T08 docs + index — connector-runtime README (3 entrypoints/Deployments, sync/async contract, at-least-once, SSRF limits, Redis parking, verify-only stream script), sdk README (invoke + invocations.get), DOCS/ (connector-vs-workflow callout, service-bus transport pair), cowork/INDEX.md registered (Engram topic platform/connector-invoke-api)
