...
Revision 4's current certification target is the human-approved integrated
commit `9fde0baa9fbae768fc97133731eeb2744f90516e`. Its source observations and
pending proof contract are recorded after the unchanged historical sections.

The attempt-1 committed review target is preserved in
...
## Revision 4 integrated target — current observations and pending proof

Revision 4 changes the certification target to the human-approved integrated
commit `9fde0baa9fbae768fc97133731eeb2744f90516e`. This section supplements the
historical T01-T05 record above. It does not reconstruct predecessor evidence,
replace the attempt-1 verdicts, or certify either the isolated or integrated
target.

### Isolated-target defect and attempt-1 verdicts retained

Commit `820221a9` is not self-consistent. Read-only inspection of that committed
Git tree shows that `.codex/manual-loop.lock.json` names six protected files,
while only `.codex/config.toml` and `.codex/agents/fp-dev.toml` exist there. The
four absent role files are:

- `.codex/agents/fp-architect.toml`
- `.codex/agents/fp-qa.toml`
- `.codex/agents/fp-reviewer.toml`
- `.codex/agents/script-runner.toml`

The lock declares `.codex/config.toml` SHA-256
`0e2aafb360454df9f511fe0ff65e0abb783ef5e533b17410d67b341e4d164e94`,
but the config bytes committed at `820221a9` hash to
`5c3cfb63a636fc7b78c6e0e6e10e67d2208c4ccd3b033167e37135b447a1ebc9`.
The committed tree also lacks `scripts/checks/check-codex-manual-loop.py`, and
its `scripts/checks/doc-code-guards.sh` lacks the G19 checker wiring. These facts
come from the committed source objects and are also preserved in
`T02-handoff.md`, `BLOCKED.md`, and the original attempt-1 packet
`T02-attempt1-certification-packet.md`.

Reviewer A discovered this dependency-boundary defect and returned REJECTED;
Reviewer B returned APPROVED. Their distinct verdicts remain verbatim in
`T02-review-verdicts.md`. The rejection is not reinterpreted or erased by the
new target, and isolated `820221a9` is not rehabilitated.

The human's root-cause finding is composition: each loop correctly committed
only its allowed paths, but a lock landed without its referents; no isolated
loop failed. This composition finding does not erase or merge the separately
recorded DEVIATION A and DEVIATION B.

### Integrated resolution in committed source objects

The human resolved the boundary by committing the pending configuration repair,
not by changing the interpretation of `820221a9`. Read-only `git log` and
`git show --stat` inspection identifies this sequence:

- `07e1d466` commits the four absent role definitions and the matching project
  config.
- `51689ecd` commits `check-codex-manual-loop.py`, its tests, and the
  `doc-code-guards.sh` wiring.
- `e111957b` commits the delivery-policy and configuration-repair records.
- `9fde0baa` commits all 82 predecessor evidence files and is the integrated
  certification target.

The exact read-only inputs are preserved as
`T02-attempt2-integrated.diff`, `T02-attempt2-integrated-log.txt`, and
`T02-attempt2-integrated-tree.txt`. The committed tree listing at `9fde0baa`
contains all six files named by `.codex/manual-loop.lock.json`, the checker,
the guard, and the checker tests. Read-only SHA-256 inspection of each of the six
committed protected-file objects matches every value declared by the lock; the
config match is
`0e2aafb360454df9f511fe0ff65e0abb783ef5e533b17410d67b341e4d164e94`.
The committed guard defines `g19_codex_manual_loop`, invokes
`python3 scripts/checks/check-codex-manual-loop.py`, and calls that function
after the KISS/full branch rejoins. These are fresh observations of committed
source objects, not historical reconstruction and not gate results.

### Pending clean-worktree evidence

The principal prepared the detached worktree
`/tmp/platform-cluster-certification-9fde0baa` at the exact target; preparation
alone is not proof. The revision-4 integrated-tree gate must still establish,
in order, an exact HEAD of
`9fde0baa9fbae768fc97133731eeb2744f90516e`, a clean worktree before the
checker, checker exit status 0, and a clean worktree afterwards. Success in the
dirty main workspace is insufficient.

The exact pending detached-worktree commands are:
...
Both cleanliness commands propagate a `git status` failure instead of treating
missing status output as a clean tree.

Those native results do not yet exist and are not claimed here. The mechanical
executor must preserve them under `T02-attempt2-*`, including the aggregate
`T02-attempt2-gates.json`, with the exact commands, working directories, output,
exit statuses, native durations, tested-state identity, and requested and
observed runner provenance. The three declared main-worktree gates precede the
detached-worktree commands, and the original T02 Accept follows them in the main
workspace. No attempt-1 gate or verdict record may be overwritten.

After all gates and Accept pass, two independent reviewers must assess both the
original historical defect and attempt-1 REJECTED/APPROVED split, and the
integrated resolution through the committed roles, config, lock, checker, guard
wiring, tests, policy, repair records, and predecessor evidence. Each reviewer
must also assess the unchanged revision-4 SPEC, this complete packet, and the
clean-worktree native evidence. Their future verdicts certify `9fde0baa` only;
the principal records the verdicts and actual provenance after review. No
certification or approval is asserted in advance. Any packet edit after the
gates invalidates those gates and both reviews.
