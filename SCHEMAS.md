...
| `createChannelEnvelope` | `services/channel-service/src/domain/envelope.factory.ts:52` | Builds a compliant inbound `ChannelEnvelope` (`kind: received` etc.) — canonical shape + legacy camelCase compatibility fields | Channel providers on ingress (webhook consumer) | Factory function (construction-time only; no post-hoc validator) |
| `createChannelSentEnvelope` | `envelope.factory.ts:147` | Builds a compliant outbound `ChannelEnvelope` (`sent`/`delivered`/`send` shadow events) | Outbound send-path code | Factory function (construction-time only) |
...
7. **Ingester classification columns — IMPLEMENTED 2026-07-10** (DECIDED 2026-07-09 — see `TAXONOMY.md` §7).
   ...
   Related finding (RESOLVED 2026-07-31, envelope-drift T08): `agent-memory-service` published `io.yoizen.agent-memory.memory.*.v1` events (until 2026-07-31 — see the type note below) on subjects built from `AGENT_MEMORY_SUBJECT_PREFIX`, which now lives in `packages/shared/src/constants.ts:145-153` beside every other internal producer's prefix (it was service-local), and the envelope's `domain` field now reads `agent-memory` — the same value as the subject's domain token — instead of the former `"automation"`. The envelope's `producer` field followed on 2026-07-31 (envelope-drift follow-up): it now reads `agent-memory-service`, matching the subject's producer token, instead of the inherited `"agent-admin-service"`. The envelope `type` followed on the same date (envelope-drift post-loop item 1): it now obeys `envelope.md:77` as `io.yoizen.agent-memory.platform.internal.<kind>.v1` (e.g. `...memory_proposed.v1`), projected from the subject constant so the two cannot drift, instead of the channel-less `io.yoizen.agent-memory.memory.*.v1` whose dotted kind (`memory.proposed`) contradicted the subject's `memory_proposed`. Events ingested before each date still carry the old values, so those columns — and `envelope->>'type'`, which `GET /events?type=` filters verbatim — span their cutover. See `DRIFT.md` discrepancy #9; the tracker classifies these by SUBJECT regardless (`TAXONOMY.md` rule 9).
