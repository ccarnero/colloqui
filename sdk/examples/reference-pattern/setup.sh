...
# =============================================================================
#
# Provisions a dedicated HTTP channel account + a workflow (echo jsFunction +
# parallel Telegram notify branch) — SDK-powered (sdk/GROWTH-PLAN.md P3.1).
# The actual provisioning logic (login, dedup, Telegram chat_id discovery,
# workflow assembly) lives in src/setup.ts, run via `@yoizen/platform-sdk`;
# this script only resolves the dev environment and execs the Node app with
# those env vars in scope.
#
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "[setup] Install Node >=18 (see sdk/examples/reference-pattern/README.md) and re-run." >&2
  exit 1
fi
if ! command -v npx >/dev/null 2>&1; then
  echo "[setup] 'npx' was not found on PATH (usually ships with npm)." >&2
  echo "[setup] Install Node >=18 / npm (see sdk/examples/reference-pattern/README.md) and re-run." >&2
  exit 1
fi
