...
import { IngestError, AuthError, ValidationError } from "../../src/domain/errors.js";
...
function setup(overrides = {}) {
  ...
  const calls = { login: 0, refresh: 0, resolve: 0, ingest: 0 };
  ...
  const auth = {
    ...
    async refresh() {
      calls.refresh++;
      if (overrides.refreshThrows) throw new AuthError("refresh failed");
      return makeToken({
  ...
  const ingest = {
    async ingest(args) {
      ...
      if (overrides.ingestImpl) return overrides.ingestImpl(args, calls);
...
test("non-accepted status surfaces as IngestError", async () => {
  ...
  await assert.rejects(
    () => client.sendText("hi"),
    (err) => err instanceof IngestError && err.ingestStatus === "no_active_accounts",
  );
});
...
test("a second signature_mismatch throws", async () => {
  ...
  await assert.rejects(
    () => client.sendText("hi"),
    (err) => err instanceof IngestError && err.ingestStatus === "signature_mismatch",
  );
});
