...
test("keeps text flat and rejects non-string text", () => {
  ...
  assert.throws(() => normalizeMessage({ from: "u", text: 5 }), ValidationError);
});
...
test("folds media and extra top-level keys into raw", () => {
  ...
  assert.deepEqual(body.raw, { media: { mimeType: "image/png" }, ticketId: 42 });
});

test("explicit raw wins over folded extras on collision", () => {
  const body = normalizeMessage({ from: "u", ticketId: 1, raw: { ticketId: 99 } });
  assert.equal(body.raw.ticketId, 99);
});
