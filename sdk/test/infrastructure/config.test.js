...
import { resolveConfig, DEFAULT_BASE_URL } from "../../src/infrastructure/config.js";
...
test("baseUrl defaults and strips trailing slashes", () => {
  ...
  assert.equal(resolveConfig({ baseUrl: "http://x/" }, FULL_ENV).baseUrl, "http://x");
  assert.equal(resolveConfig({ baseUrl: "http://x///" }, FULL_ENV).baseUrl, "http://x");
});

test("defaultFrom defaults to email", () => {
  const cfg = resolveConfig({ tenant: "t", email: "e@x.com", password: "p" }, {});
