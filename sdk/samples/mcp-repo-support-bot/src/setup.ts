...
const MCP_SERVER_DESCRIPTION =
  ...
  "DeepWiki public MCP server — answers questions about indexed GitHub repos. Registered by sdk/samples/mcp-repo-support-bot.";
...
async function stageUpsertAgents(): Promise<void> {
  ...
  triageAgentId = await upsertAndPublishAgent(
    ...
    buildAgentPayload(
      TRIAGE_AGENT_NAME,
      `Classifies whether an inbound message is a question about ${REPO_NAME}. Created by sdk/samples/mcp-repo-support-bot.`,
  ...
  summarizerAgentId = await upsertAndPublishAgent(
    ...
    buildAgentPayload(
      SUMMARIZER_AGENT_NAME,
      `Rewrites DeepWiki's technical answer about ${REPO_NAME} into a brief, friendly reply. Created by sdk/samples/mcp-repo-support-bot.`,
