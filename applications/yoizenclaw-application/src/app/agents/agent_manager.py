...
from src.utils.config.channel_config import ChannelConfigStore
...
class AgentManager:
    ...
    async def update_agent_config(
        ...
        """Update the selected agent configuration at runtime."""

        await self._agent_store.save_active_config(config, agent_id)
        await self._reload()
    ...
    async def _reload(self) -> None:
        ...
        self._agent_config_cache = {
            self._cache_key(aid): cfg for aid, cfg in all_agent_configs.items()
        }
    ...
    def _build_agent_instance(self, agent_config: dict[str, Any]) -> Agent:
        """Build an agent instance, using EnhancedAgent if enhanced features are enabled."""
        # Check if enhanced features are enabled
        enable_enhanced = agent_config.get("enable_enhanced_skills", True)

        if enable_enhanced:
            try:
                # Try to parse as enhanced agent config
                enhanced_config = EnhancedAgentSyncRequest(**agent_config)
                return self._build_enhanced_agent_instance(enhanced_config)
            except Exception as e:
                logger.warning(
                    f"Failed to create enhanced agent, falling back to legacy: {e}"
                )
    ...
    def _build_enhanced_agent_instance(
        self, enhanced_config: EnhancedAgentSyncRequest
    ) -> EnhancedAgent:
        """Build an enhanced agent instance with new skill system."""
        from src.app.agents.enhanced_agent import EnhancedAgent

        config_dict = enhanced_config.to_runtime_dict()
        ...
        return EnhancedAgent(
            ...
            tool_registry=get_registry(),
            agent_metadata={
                "id": config_dict.get("id", self._default_agent_id or ""),
    ...
    def _raise_not_configured(self) -> None:
        from src.utils.errors import RuntimeNotConfiguredError
