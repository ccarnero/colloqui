...
def _map_status_to_model(status_str: str) -> "JobExecutionStatus":
    from src.jobs.domain.entities import JobExecutionStatus
...
def _row_to_job_execution(row: object) -> Optional["JobExecution"]:
    from src.jobs.domain.entities import JobExecution
...
async def get_job(
    ...
    from src.jobs.domain.entities import JobDefinition, RetryPolicy
