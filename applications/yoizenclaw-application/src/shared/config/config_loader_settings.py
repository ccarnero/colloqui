...
from src.shared.config.loader import ConfigLoader
from src.shared.config.runtime_paths import (
...
class GlobalSettings:
    """Alternative singleton-style access to settings.
    
    Provides instance-based access pattern as an alternative
    to the class-level Settings API.
    """
    
    _instance: Optional["GlobalSettings"] = None
    
    def __new__(cls) -> "GlobalSettings":
        if cls._instance is None:
            cls._instance = super().__new__(cls)
            cls._instance._loader = ConfigLoader(_default_config_dir())
            cls._instance._initialized = False
        return cls._instance
    
    @property
    def loader(self) -> ConfigLoader:
        """Access the underlying ConfigLoader."""
        return self._loader
    
    def get(self, key: str, default: Any = None) -> Any:
        """Get a configuration value."""
        if not self._initialized:
            self._loader.preload_configs("settings.yaml")
            self._initialized = True
        return self._loader.get(key, default, "settings.yaml")
