...
# =============================================================================
# dev-mode.sh — source-mounted developer mode for Yoizen platform services
# =============================================================================
#
# Flips a running service to live-reload mode: swaps its image for
# oven/bun:1.3.14-slim, mounts the repo root from the host (hostPath) and
# pre-installed node_modules from a shared PVC, then runs `bun --watch`.
# File changes on the host are visible immediately via OrbStack VirtioFS —
# no polling, no image rebuild required.
#
# Usage:
#   ./dev-mode.sh deps [--force]             # ensure PVC, populate node_modules
#   ./dev-mode.sh <service> on               # flip service to dev mode
#   ./dev-mode.sh <service> off [--overlay postgres-dev|mongo-dev]
#                                            # restore declared state
#   ./dev-mode.sh <service> status           # show dev-mode state for one service
#   ./dev-mode.sh status                     # show dev-mode state for all services
#
# Supported services:
...
#
# Prerequisites:
#   - OrbStack Kubernetes cluster (context: orbstack)
#   - jq on PATH (required for JSON patch operations)
#   - kubectl on PATH
#   - Cluster bootstrapped with ./bootstrap-orbstack-osx.sh
#   - Run ./dev-mode.sh deps before first use of `on`
# =============================================================================
...
REPO_PATH="/Users/chris/sources/yoizen/platform-cluster"
...
check_deps() {
  ...
  if (( ${#missing[@]} )); then
    ...
    err "Install jq with: brew install jq"
    exit 1
  fi
}
...
check_context() {
  ...
  if [[ "$ctx" != "orbstack" ]]; then
    warn "Current kubectl context is '${ctx}', expected 'orbstack'"
    warn "Switch with: kubectl config use-context orbstack"
...
get_targets() {
  ...
  case "$svc" in
    ...
    *)
      ...
      err "Run ./dev-mode.sh status to see supported services."
      exit 1
      ;;
  esac
}
...
build_patch_ops() {
  ...
  # ── command ──
  local cmd_json
  cmd_json="$(printf '["bun","--watch","%s"]' "$entrypoint")"
...
cmd_deps() {
  ...
  local job_output
  job_output="$(kubectl create -f "${SCRIPT_DIR}/knative/dev-mode/deps-install-job.yaml")"
  local job_name
...
cmd_status() {
  ...
  if [[ -z "$cluster_sha" ]]; then
    warn "Deps: not installed (run ./dev-mode.sh deps)"
  elif [[ "$local_sha" == "$cluster_sha" ]]; then
    log "Deps: up to date (sha: ${cluster_sha:0:12}...)"
  else
    warn "Deps: STALE — lock sha changed (run ./dev-mode.sh deps)"
  ...
  for svc in "${found_svcs[@]}"; do
    ...
    while IFS=$'\t' read -r kind name entrypoint svc_dir; do
      [[ -z "$name" ]] && continue
      if [[ "$kind" == "ksvc" ]] && printf '%s\n' "$dev_ksvcs" | grep -qx "$name"; then
        printf "      ksvc/%-30s  bun --watch %s\n" "$name" "$entrypoint"
      elif [[ "$kind" == "deploy" ]] && printf '%s\n' "$dev_deploys" | grep -qx "$name"; then
        printf "      deploy/%-28s  bun --watch %s\n" "$name" "$entrypoint"
      fi
    done <<< "$targets"
  done
...
Commands:
  ...
  <service> on
      Switch service to source-mounted dev mode (bun --watch).
