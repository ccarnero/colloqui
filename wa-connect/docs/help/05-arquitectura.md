...
│   │   │   └── template-routes.js
│   │   ├── ws/
│   │   │   └── create-ws-server.js ← WebSocket con auth JWT
│   │   └── scripts/
...
│       │   └── use-websocket.js  ← Hook con auth + auto-reconnect
│       ├── pages/
│       │   ├── LoginPage.jsx
│       │   ├── RegisterPage.jsx
...
webhook-routes.js → responde 200 inmediato
      ↓
parseWebhook() → normaliza el payload
      ↓
parseSenderId() → detecta phone/bsuid
      ↓
Busca account por phone_number_id
      ↓
upsertContact() → crea o actualiza contacto
      ↓
saveMessage() → guarda en MongoDB
      ↓
broadcast() → WebSocket a dashboards conectados
```
...
message-routes.js → busca account → obtiene token
      ↓
sendText(token, phoneNumberId, to, text)
      ↓ (en LIVE mode)
fetch → graph.facebook.com/v22.0/{phone_id}/messages
      ↓
Guarda mensaje outbound en MongoDB
      ↓
Respuesta al frontend con el mensaje guardado
```
...
Actualmente 7 archivos de test, 32 tests.
Los tests cubren los módulos puros (result, pipe, match, parseSenderId, parseWebhook, verifyWebhook, exchangeToken). Los tests de DB/routes requieren MongoDB y están pendientes para una futura iteración con testcontainers o similar.
