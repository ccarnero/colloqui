...
En modo dev (Vite en :5173), el proxy solo funciona para HTTP, no para WebSocket. El hook `use-websocket.js` detecta esto y conecta directo a `:6666`:
...
Si el WS no conecta en dev, verificá que el server esté en `:6666`.
...
6. cloudflared tunnel run wa-connect (o --url http://localhost:6666)
