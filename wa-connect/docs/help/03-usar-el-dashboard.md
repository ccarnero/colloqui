...
```
┌──────────────────────────────────────────────────────┐
│ WA Connect    Business Name      + Add Account  Chris│  ← Header
├──────────────┬───────────────────────────────────────┤
│              │                                       │
...
El dashboard se conecta por WebSocket al servidor. Cuando llega un mensaje nuevo:
...
Si la conexión WebSocket se pierde, se reconecta automáticamente cada 3 segundos. Mientras tanto, podés usar el botón de refresh.
...
1. Un cliente te escribe por WhatsApp
      ↓
2. El mensaje llega al webhook → se guarda en MongoDB → se envía por WebSocket
      ↓
...
WA Connect soporta múltiples cuentas de WhatsApp Business. Cada cuenta tiene sus propias conversaciones y contactos, aisladas entre sí.

Para agregar otra cuenta: clic en "+ Add Account" en el header.

(Nota: en el MVP actual no hay un selector visual de cuenta en el sidebar — usa la primera cuenta conectada. El selector multi-cuenta viene en una iteración futura.)
