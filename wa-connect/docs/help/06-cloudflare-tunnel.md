...
```bash
cloudflared tunnel create wa-connect
```
...
cloudflared tunnel route dns wa-connect api.devmachina.net
```
...
```bash
cloudflared tunnel run wa-connect
```
...
ATENCIÓN con la ruta: en el proyecto anterior era `/webhook`. En WA Connect es `/api/webhooks/whatsapp`. Si la ponés mal, Meta va a devolver error de verificación.
...
```
✓ cloudflared tunnel run wa-connect está corriendo
...
2. Levantá el túnel: `cloudflared tunnel run wa-connect`
3. Abrí WA Connect en el browser (http://localhost:5173 en dev)
...
```
✓ cloudflared está corriendo (cloudflared tunnel run wa-connect)
...
✓ Si persiste: cloudflared tunnel cleanup wa-connect y volver a correr
...
# o crear uno nuevo (wa-connect) y rutear el DNS al nuevo
cloudflared tunnel route dns wa-connect api.devmachina.net
```
