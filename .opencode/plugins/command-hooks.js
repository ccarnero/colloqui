...
// src/execution/shell.ts
import { execFile } from "child_process";
var DEFAULT_TRUNCATE_LIMIT = 30000;
...
async function executeCommands(commands, hookId, options) {
...
var executeShellCommand = async (command) => {
...
// src/execution/template.ts
var replacePlaceholder = (template, placeholder, value) => {
  const stringValue = value === undefined || value === null ? "" : String(value);
  const regex = new RegExp(`\\{${placeholder}\\}`, "g");
  return template.replace(regex, stringValue);
};
var interpolateTemplate = (template, context) => {
  if (!template) {
    return "";
  }
  if (isDebugEnabled()) {
...
var filterSessionHooks = (hooks, criteria) => {
...
var filterToolHooks = (hooks, criteria) => {
  return hooks.filter((hook) => {
...
var formatErrorMessage = (hookId, error) => {
...
var showToast = async (client, title, message, variant = "info", duration) => {
  try {
...
var injectMessage = async (client, sessionId, message) => {
  try {
...
var executeHook = async (hook, context, client, truncationLimit) => {
  ...
  try {
    ...
    try {
      ...
      const injectionErrorMsg = injectionError instanceof Error ? injectionError.message : String(injectionError);
...
async function executeHooks(hooks, context, client, truncationLimit) {
  try {
    logger.debug(`executeHooks called with ${hooks.length} hook(s) for session "${context.sessionId}", truncationLimit: ${truncationLimit}`);
...
var NEVER = INVALID;
// src/schemas.ts
var StringOrArray = exports_external.union([exports_external.string(), exports_external.array(exports_external.string())]);
var PhaseSchema = exports_external.enum(["before", "after"]);
var SessionEventSchema = exports_external.enum(["session.created", "session.idle", "session.end", "session.start"]);
var ToolHookWhenSchema = exports_external.object({
  phase: PhaseSchema,
  tool: StringOrArray.optional(),
  callingAgent: StringOrArray.optional(),
  slashCommand: StringOrArray.optional()
});
var ToastSchema = exports_external.object({
  title: exports_external.string().optional(),
  message: exports_external.string(),
  variant: exports_external.enum(["info", "success", "warning", "error"]).optional(),
  duration: exports_external.number().optional()
}).optional();
var ToolHookSchema = exports_external.object({
  id: exports_external.string().min(1, "Hook ID must not be empty"),
  when: ToolHookWhenSchema,
  run: exports_external.union([exports_external.string(), exports_external.array(exports_external.string())]),
  inject: exports_external.string().optional(),
  toast: ToastSchema
});
var SessionHookWhenSchema = exports_external.object({
  event: SessionEventSchema,
  agent: StringOrArray.optional()
});
var SessionHookSchema = exports_external.object({
  id: exports_external.string().min(1, "Hook ID must not be empty"),
...
var ConfigSchema = exports_external.object({
  truncationLimit: exports_external.number().int().positive().optional(),
  tool: exports_external.array(ToolHookSchema).optional(),
  session: exports_external.array(SessionHookSchema).optional()
});
var isValidCommandHooksConfig = (value) => {
...
import { access as access2, constants as constants2, readFile as readFile2 } from "fs/promises";
var extractYamlFrontmatter = (content) => {
...
var parseYamlFrontmatter = (content) => {
  try {
...
var parseAgentHooks = (yamlContent) => {
...
var convertToCommandHooksConfig = (agentHooks, agentName) => {
  const toolHooks = [];
...
var convertAgentHookEntryToToolHook = (entry, agentName, phase, index) => {
  if (!entry.run) {
...
var extractAgentNameFromPath = (filePath) => {
  const fileName = filePath.split("/").pop() || "";
  return fileName.replace(/\.md$/, "");
};
var loadMarkdownConfig = async (filePath) => {
  try {
    let content;
    try {
    ...
    if (agentHooks) {
      const result2 = convertToCommandHooksConfig(agentHooks, agentName);
      logger.debug(`Loaded simplified hooks from ${filePath}: ${result2.tool?.length ?? 0} tool hooks`);
      return result2;
    }
...
// src/config/agent.ts
async function resolveAgentPath(agentName) {
  ...
  const projectAgentPath = join2(process.cwd(), ".opencode", "agent", agentFileName);
  try {
    await access3(projectAgentPath, constants3.F_OK);
  ...
  const userAgentPath = join2(homedir(), ".config", "opencode", "agent", agentFileName);
  try {
    await access3(userAgentPath, constants3.F_OK);
...
// src/config/merge.ts
var findDuplicateIds = (hooks) => {
  const idCounts = new Map;
  for (const hook of hooks) {
    const count = idCounts.get(hook.id) ?? 0;
    idCounts.set(hook.id, count + 1);
  }
  return Array.from(idCounts.entries()).filter(([, count]) => count > 1).map(([id]) => id);
};
var validateConfigForDuplicates = (config, source) => {
  const errors2 = [];
...
var mergeHookArrays = (globalHooks, markdownHooks) => {
...
var mergeConfigs = (global, markdown) => {
  ...
  logger.debug(`Merged configs: ${mergedToolHooks.length} tool hooks, ${mergedSessionHooks.length} session hooks, truncationLimit: ${mergedConfig.truncationLimit}, ${errors2.length} errors`);
  return { config: mergedConfig, errors: errors2 };
};

// src/index.ts
var notifyConfigError = async (configError, sessionId, client) => {
  if (!configError)
    return;
...
var toolCallArgsCache = new Map;
var notifiedConfigErrors = new Set;
var storeToolArgs = (callId, args) => {
  if (!callId || !args)
    return;
  toolCallArgsCache.set(callId, args);
};
var getToolArgs = (callId) => {
  if (!callId)
    return;
  return toolCallArgsCache.get(callId);
};
var deleteToolArgs = (callId) => {
  if (!callId)
    return;
  toolCallArgsCache.delete(callId);
};
var handleSessionEvent = async (eventType, sessionId, agent, client) => {
  if (!sessionId) {
...
var handleToolExecutionHook = async (phase, input, toolArgs, client) => {
  try {
    const { config: globalConfig, error: globalConfigError } = await loadGlobalConfig();
...
var CommandHooksPlugin = async ({ client }) => {
